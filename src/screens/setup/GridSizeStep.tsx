...
export default function GridSizeStep({ navigation, route }: any) {
  ...
  const handleSelect = async (gridSize: number) => {
    setLoading(true);
    try {
      const config: GameConfig = { gridSize, winLength: gridSize, maxPieces: gridSize, difficulty: difficulty || 'auto' };
      if (mode === 'friend') {
        const matchId = await createMatch(user!.uid, user!.displayName || 'Player', config);
        navigation.dispatch(CommonActions.navigate('Main', { screen: 'Friends', params: { pendingMatchId: matchId } }));
      } else {
            ...
            >
              <View style={[
                ...
                isCalm 
                  ...
                  : { backgroundColor: t.secondary + '22', borderColor: t.secondary + '44' }
              ]}>
                {loading
                  ? <ActivityIndicator color={isCalm ? t.primary : t.secondary} />
                  : <Ionicons name={opt.icon as any} size={24} color={isCalm ? t.primary : t.secondary} />
                }
              </View>
              ...
              {!loading && <Ionicons name="chevron-forward" size={20} color={t.textSecondary} />}
            </Pressable>
          </Animated.View>
        ))}

        <View style={s.badge}>
          <Ionicons name="flash" size={12} color={isCalm ? t.primary : t.warning} />
          <Text style={[s.badgeText, { color: isCalm ? t.primary : t.warning }]}>AUTO-CONFIG ACTIVE</Text>
        </View>
      </View>
    </SetupStepWrapper>
  );
...
const s = StyleSheet.create({
  ...
  badge:       { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: Spacing.md, opacity: 0.65 },
